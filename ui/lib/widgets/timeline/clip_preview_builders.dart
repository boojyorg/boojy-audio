...
mixin ClipPreviewBuildersMixin on State<TimelineView>, TimelineViewStateMixin {
  ...
  List<Widget> buildCopyDragPreviews(
    MidiClipData sourceClip,
    Color trackColor,
    double trackHeight,
  ) {
    ...
    final newStartBeats = (sourceClip.startTime + snappedDeltaBeats).clamp(
      0.0,
      double.infinity,
    );
  ...
  List<Widget> buildAudioCopyDragPreviews(
    ClipData sourceClip,
    Color trackColor,
    double trackHeight,
  ) {
    ...
    // Apply the snapped delta to this source clip
    final newStartTime = (sourceClip.startTime + snappedDeltaSeconds).clamp(
      0.0,
      double.infinity,
    );
                        ...
                        Expanded(
                          child: LayoutBuilder(
                            builder: (context, constraints) {
                              ...
                              final clipGainDb =
                                  sourceClip.editData?.gainDb ?? 0.0;
  ...
  List<Widget> buildAudioCopyDragPreviewsForMidiDrag(
    ClipData sourceClip,
    Color trackColor,
    double trackHeight,
  ) {
  ...
  List<Widget> buildMidiCopyDragPreviewsForAudioDrag(
    MidiClipData sourceClip,
    Color trackColor,
    double trackHeight,
  ) {
  ...
  Widget buildPreviewClip(PreviewClip preview) {
    ...
    final trackHeight =
        widget.trackHeightState.clipHeights[preview.trackId] ??
                          ...
                          builder: (context, constraints) {
                            if (preview.isMidi) {
                              if (preview.midiNotes == null ||
                                  preview.midiNotes!.isEmpty) {
                                return const SizedBox();
                              }
                              final durationBeats =
                                  previewDuration * (widget.tempo / 60.0);
                              return CustomPaint(
                                size: Size(
                                  constraints.maxWidth,
                                  constraints.maxHeight,
                              ...
                              if (preview.waveformPeaks == null ||
                                  preview.waveformPeaks!.isEmpty) {
                                return const SizedBox();
                              }
                              return CustomPaint(
                                size: Size(
